...
def main(frame, bone, aug1, aug2):
    ...
    parser.add_argument('--framework', type=str, default=frame)
    parser.add_argument('--backbone', type=str, default=bone)
    parser.add_argument('--aug1', type=str, default=aug1)
    parser.add_argument('--aug2', type=str, default=aug2)

    torch.manual_seed(10)
    np.random.seed(10)
    args = parser.parse_args()
    DEVICE = torch.device('cuda:' + str(args.cuda) if torch.cuda.is_available() else 'cpu')
    print('device:', DEVICE, 'dataset:', args.dataset, 'framework:', args.framework, 'backbone:', args.backbone, 'aug1:', args.aug1, 'aug2:', args.aug2)
...
if __name__ == '__main__':
    ...
    for frame in framework:
        for bone in backbone:
            for a1 in aug1:
                for a2 in aug2:
                    toTest.append([frame, bone, a1, a2])
                    count = count + 1

    start = 0 # the start of where to run from
    
    for index in range(start, count):
        main(toTest[index][0], toTest[index][1], toTest[index][2], toTest[index][3])