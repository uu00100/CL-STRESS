...
from data_preprocess import data_preprocess_lab_v2
from data_preprocess import data_preprocess_beam
...
global matrix_dir_name
matrix_dir_name = 'matrix'
if not os.path.exists(matrix_dir_name):
    os.makedirs(matrix_dir_name)

def setup_dataloaders(args):
    if args.dataset == 'beam':
        args.n_feature = 9
        args.len_sw = 128
        args.n_class = 5 # types of states for the beam
        if args.cases not in ['subject', 'subject_large']:
            args.target_domain == '0'
        train_loaders, val_loader, test_loader = data_preprocess_beam.prep_ucihar(args, SLIDING_WINDOW_LEN=args.len_sw, SLIDING_WINDOW_STEP=int(args.len_sw * 0.5))
    if args.dataset == 'lab':
        args.n_feature = 9
        args.len_sw = 128
        args.n_class = 2 # number of data types
        if args.cases not in ['subject', 'subject_large']:
            args.target_domain == '0'
        train_loaders, val_loader, test_loader = data_preprocess_lab_v2.prep_ucihar(args, SLIDING_WINDOW_LEN=args.len_sw, SLIDING_WINDOW_STEP=int(args.len_sw * 0.5))
...
def setup(args, DEVICE):
    ...
    args.matrix_name = args.dataset + '_' + args.framework + '_' + args.backbone + '_' + args.aug1 + '_' + args.aug2 + '_' + str(args.n_epoch) + '.txt'
...
def test_lincls(test_loader, trained_backbone, best_lincls, logger, fitlog, DEVICE, criterion, args, plt=False):
    ...
    np.savetxt(matrix_dir_name + '/' + args.matrix_name, confusion_matrix.numpy())
